...
int gameFlag=0;//游戏开始标志位
StructGame gameEndFlag;//游戏结果
int main(void)
{
	structData numData;
	delay_init();	    	 //延时函数初始化	  
    ...
    numData.data2=date3;		
		...
		/*-----------------游戏模式---------------------*/
		if(gameFlag==1&&studyNumFlag==0&&studyHandFlag==0)//游戏开始判断条件
		{
			 gameEndFlag=gameEnd(numData);
		}
	}
}
/**
