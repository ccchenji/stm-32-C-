...
//DRIVER_DIR   PA0 
//DRIVER_OE    PA2 
//STEP_PULSE   PA8 (TIM1_CH1,LCD_RW)
******************************************/
...
void Driver_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_2;	//PA0.2 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		//IO口速度为50MHz
	GPIO_Init(GPIOA, &GPIO_InitStructure);					//根据设定参数初始化GPIOA
	GPIO_SetBits(GPIOA,GPIO_Pin_0);						 	//PA0输出高 顺时针方向  DRIVER_DIR
	GPIO_ResetBits(GPIOA,GPIO_Pin_2);						//PA2输出低 使能输出  DRIVER_OE
}

/***********************************************
//TIM1_CH1(PA8) 单脉冲输出+重复计数功能初始化
//TIM1 时钟频率 72MHz
//arr：自动重装值
//psc：时钟预分频数
************************************************/
...
void Locate_Abs(long num,u32 frequency)//绝对定位函数
{
	...
	if(target_pos!=current_pos)//目标和当前位置不同
	{
		...
		is_rcr_finish=0;//重复计数器未设置完成
		if(abs(target_pos-current_pos)%(RCR_VAL+1)>0.001)
		{
		TIM1_Startup(frequency);//开启TIM1
		}
	}
